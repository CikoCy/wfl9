...
# === Parametri LSTM ===
TIMESTEPS = 10     # Quante estrazioni passate usare come input
FEATURES = 20      # 20 numeri da 1 a 20 → codificati one-hot
...
def carica_dati_lstm(path=STORICO_PATH):
    ...
    # ✅ Filtra solo righe REALE nella colonna Topo
    ...
    for i in range(TIMESTEPS, len(df)):
        ...
        # Estrai X = sequenze di estrazioni precedenti
        X_seq = np.array([
            one_hot_encode_estrazione(eval(r["10 Numeri"]))
            for _, r in blocco.iterrows()
        ])

        # Estrai y = 10 numeri + numerone normalizzati
        y = df.iloc[i]
        y_estratti = eval(y["10 Numeri"])
        y_numerone = int(y["Numerone"])
        y_output = np.zeros(11)
        for idx, n in enumerate(y_estratti):
            if 1 <= n <= 20:
                y_output[idx] = (n - 1) / 19  # Normalizza tra 0–1
